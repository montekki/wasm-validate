use self::common::stack;
use self::context::{ModuleContext, ModuleContextBuilder};
use core::fmt;
use memory_units::{Bytes, Pages};
use parity_wasm::elements::{
    BlockType, External, Func, FuncBody, GlobalEntry, GlobalType, InitExpr, Instruction, Internal,
...
pub mod common;
pub mod context;
pub mod fp_validator;
pub mod func;

mod util;

use func::DefaultFuncValidator;
...
pub struct Validator<'a, 'b: 'a> {
    func_validators: Vec<&'a mut FunctionValidator<'b>>,
}

impl<'a, 'b: 'a> Validator<'a, 'b> {
    pub fn new() -> Validator<'a, 'b> {
        let v = Validator {
            func_validators: vec![],
        };
        v
    ...
    pub fn add_validator(&mut self, val: &'a mut FunctionValidator<'b>) {
        self.func_validators.push(val);
    }

    pub fn validate_module(&mut self, module: &'b Module) -> Result<(), Error> {
        let mut context_builder = ModuleContextBuilder::new();
        let mut imported_globals = Vec::new();
        //let mut code_map = Vec::new();
        println!("validate_module");
        ...
        if function_section_len != 0 {
            ...
            for (index, function) in function_section.entries().iter().enumerate() {
                ...
                let mut dv = DefaultFuncValidator::new();
                dbg!(self.read_function(&context, function, function_body, &mut dv))?;
    ...
    fn read_function(
        ...
        func: &'b Func,
        body: &'b FuncBody,
        _dv: &mut DefaultFuncValidator,
    ) -> Result<(), Error> {
        for v in self.func_validators.iter_mut() {
            dbg!(v.begin_function(context, func, body))?;
        }

        for (position, instruction) in body.code().elements().iter().enumerate() {
            for v in self.func_validators.iter_mut() {
                v.next_instruction(context, &instruction, position)?;
            }
        }
